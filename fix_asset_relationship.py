...
def fix_asset_relationship():
    ...
    logger.info("Starting fix of Asset model relationship...")
    ...
    for i, line in enumerate(content):
        ...
        if 'transactions = relationship("AssetTransaction"' in line and line.strip()[-1] != ')':
            logger.info("Found problematic line at line {i+1}: {line.strip()}")
            ...
            if '"desc(AssetTransaction.transaction_date)' in line and not line.strip().endswith('"'):
                ...
                logger.info("Fixed line: {content[i].strip()}")
                fixed = True
    ...
    if not fixed:
        ...
        for i, line in enumerate(content):
            if pattern.search(line) and not line.strip().endswith(')'):
                logger.info("Found problematic relationship line at line {i+1}: {line.strip()}")
    ...
    if not fixed:
        logger.info("Could not automatically fix the issue. Please check the file manually.")
        logger.info("Look for unterminated strings near line 78 in {model_file_path}")
        return False
    ...
    try:
        ...
        logger.info("Successfully fixed relationship definition in Asset model at {model_file_path}")
        return True
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if success:
        ...
        logger.info("Don't forget to restart your application with:")
