...
def update_asset_model():
    print(f"Updating Asset model in {ASSET_MODEL}")
    ...
    if "ticket_assets = Table(" in asset_content:
        print("ticket_assets table is already defined in Asset model")
    else:
...
"""
    ...
    if "tickets = relationship" in asset_content:
        print("tickets relationship is already defined in Asset model")
    else:
        ...
        if re.search(relationship_pattern, asset_content, re.DOTALL | re.MULTILINE):
            ...
            print("Relationship section not found, trying to add at end of class")
    ...
    print(f"Updated {ASSET_MODEL}")
    return True

# Update Ticket model to add assets relationship
def update_ticket_model():
    print(f"Updating Ticket model in {TICKET_MODEL}")
    if not os.path.exists(TICKET_MODEL):
        print(f"Error: {TICKET_MODEL} not found!")
        return False
    ...
    if "assets = relationship" in ticket_content:
        print("assets relationship is already defined in Ticket model")
        return True
    ...
    if re.search(relationship_pattern, ticket_content, re.DOTALL):
        ...
        print("Could not find proper insertion point for assets relationship")
        return False
...
try:
    ...
    if asset_updated and ticket_updated:
        print("Successfully updated both models. Please restart your web application.")
        print("If the changes don't take effect, check the backup files and manually merge the changes.")
        ...
        print("WARNING: Not all models were updated successfully!")
