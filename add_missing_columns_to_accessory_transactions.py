...
def run_migration():
    ...
    if not cursor.fetchone():
        logger.info("Table 'accessory_transactions' does not exist. Please create it first.")
        conn.close()
        return False
    ...
    logger.info("Existing columns: {existing_columns}")
    ...
    if 'user_id' in added_columns:
        try:
            ...
            logger.info("Created index for user_id")
        except sqlite3.OperationalError as e:
            logger.info("Error creating index for user_id: {str(e)}")
    ...
    if added_columns:
        logger.info("Successfully added columns: {', '.join(added_columns)} at {datetime.now()}")
        return True
    else:
